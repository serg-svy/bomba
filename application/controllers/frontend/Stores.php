...
class Stores extends FrontEndController
{
    private $page_id;
    ...
    public function index() {
        ...
        $all_cities = $this->cities_model->get_cities_for_pickup($this->lclang);
        if(uri(3)) {
            $stores = $this->stores_model->get_stores($this->lclang, uri(3));
            $current_cities = $this->cities_model->get_cities_for_pickup($this->lclang, uri(3));

            $this->breadcrumbs[] = $this->_generate_bc_data($current_cities[0]->title, $current_cities[0]->id);

            foreach(language(true) as $lang){
                $array = $lang.'_urls';
                $this->{$array}[] = $current_cities[0]->id;
            }
        } else {
            $stores = $this->stores_model->get_stores($this->lclang);
            $current_cities = $all_cities;
        }

        $this->loadOGImgData($page);
        $this->_init_seo_data($page);

        $this->data['inner_view'] = 'pages/stores/index';
        $this->data['page'] = $page;
        $this->data['stores'] = $stores;
        $this->data['all_cities'] = $all_cities;
        $this->data['current_cities'] = $current_cities;
