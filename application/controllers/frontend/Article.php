...
class Article extends FrontEndController
{
    ...
    public function index() {
        ...
        $start = ($get['page']-1)*ARTICLES_PER_PAGE;

        $this->load->model("article_model");
        $articles = $this->article_model->get_articles($this->clang, $start, ARTICLES_PER_PAGE);
        $count = $this->article_model->get_articles_count();

        $articles = array_chunk($articles, 4);

        $uri_parts = explode('?', $_SERVER['REQUEST_URI'], 2);
        $urlPattern = $uri_parts[0].'?page=(:num)';

        $paginator = new Paginator($count, ARTICLES_PER_PAGE, $get['page'], $urlPattern);
        $paginator->setMaxPagesToShow(5);
        $this->data['paginator'] = $paginator;

        $this->data['inner_view'] = 'pages/article/index';
        $this->data['page'] = $page;
    ...
    public function item() {
        ...
        $this->load->model("article_model");
        $new = $this->article_model->get_article_by_uri($this->lclang, uri(3));
        ...
        $products = [];
        $request = $this->db->where("article_id", $new->id)->get("article_product")->result();
        if($request) {
        ...
        $lastArticles = $this->article_model->get_last_articles($this->clang, $new->id);

        $this->data['inner_view'] = 'pages/article/item';
        $this->data['page'] = $new;
