...
$head1='блоки на главной';
$head2='Редактирование баннера';
$head3='name_ru';
$addnew='блок';
$tblname='main_blocks';
$headerloc='main_blocks';
...
$form[]=admin_form_text(true,'url_name','Название кнопки');
$form[]=admin_form_file(false,'image','Изображение 272px x 485px',$tblname);
$form[]=admin_form_file(false,'image_mobile','Изображение 395px x 575px',$tblname);
...
<?if (empty($uri3)) {?>
                ...
                <div class="tab-pane active" id="tab_1_1">
                    <div class="">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tr>
                                <td width="200">Позиция</td>
                                <td>
                                    <select class="form-control" name="data[position]">
                                        <option <?=('1'==$data['position']) ? 'selected' : '' ?> value="1">Первый ряд</option>
                                        <option <?=('2'==$data['position']) ? 'selected' : '' ?> value="2">Второй ряд</option>
                                        <option <?=('3'==$data['position']) ? 'selected' : '' ?> value="3">третий ряд</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
    ...
    <script>
        function addProd() {
            ...
            $.get('/<?=ADM_CONTROLLER?>/main_blocks_product/'+main_block_id+'/'+product_id+'/',function(data) {
        ...
        function delProd(main_block_id,product_id) {
            if(confirm('Вы уверенны? ')){
                $.get('/<?=ADM_CONTROLLER?>/main_blocks_product_del/' + main_block_id + '/' + product_id + '/', function (data) {
                    $('#prod_table').html(data);
                });
            }
        }
        ...
        $(document).ready(function() {
            ...
            $.get('/<?=ADM_CONTROLLER?>/main_blocks_product/'+main_block_id+'/',function(data) {
                $('#prod_table').html(data);
            });
