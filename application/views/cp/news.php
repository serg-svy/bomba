...
$form[] = admin_form_text(true, 'name', 'Заголовок1');
...
<? if (empty($uri3)) { ?>
    ...
    <script>
        function addProd() {
            ...
            $.get('/<?=ADM_CONTROLLER?>/news_product/'+news_id+'/'+product_id+'/',function(data) {
        ...
        function delProd(news_id,product_id) {
            if(confirm('Вы уверенны? ')){
                $.get('/<?=ADM_CONTROLLER?>/news_product_del/' + news_id + '/' + product_id + '/', function (data) {
                    $('#prod_table').html(data);
                });
            }
        }
        ...
        $(document).ready(function() {
            ...
            $.get('/<?=ADM_CONTROLLER?>/news_product/'+news_id+'/',function(data) {
                $('#prod_table').html(data);
            });
