...
$head1='Баннеры в категориях';
$head2='Редактирование баннера';
$head3='text1';
$addnew='баннер';
$tblname='category_banners';
$headerloc='category_banners';
...
$form[]=admin_form_select('type_id','Тип баннера','banner_type','name');
...
<?if (empty($uri3)) {?>
	...
	<?
	$typevals=$this->db->get('banner_type')->result_array();
		...
		<div class="tab-content">
			...
			<div class="tab-pane" id="tab_1_2">
				...
				<div style="width:calc(100% - 150px);float:left;">
					...
					<?
					$prod = $this->db->select('name_ru,id,level')->order_by('level asc,name_ru asc')->get('category')->result_array();
					foreach($prod as $row) {
						echo '<option value="'.$row['id'].'">'.$row['name_ru'].' (уровень '.$row['level'].')</option>';
					}
					?>
					</select>
				</div>
	...
	function addProd() {
		...
		$.get('/<?=ADM_CONTROLLER?>/bannerCat/'+banner+'/'+cat+'/',function(data) {
			$('#prod_table').html(data);
		});
	}

	function bannerCatDel(promo,prod) {
		$.get('/<?=ADM_CONTROLLER?>/bannerCatDel/'+promo+'/'+prod+'/',function(data) {
			$('#prod_table').html(data);
		});
	}
	...
	$(document).ready(function() {
		...
		$.get('/<?=ADM_CONTROLLER?>/bannerCat/'+promo+'/',function(data) {
			$('#prod_table').html(data);
		});
