...
<section class="listing">
    <div class="wrapper">
        <div class="listing__sides listing__sides171">
            ...
            <div class="side__rht">
                <?php if(isset($find_categories) and !empty($find_categories) and !empty($query)){?>
                    <div class="categories__block categories__block-2">
                        <?php foreach($find_categories as $category) {?>
                            <a href="/<?=$lclang?>/<?=$menu['all'][11]->uri?>/<?=$category['uri']?>/?query=<?=$query?>" class="categories__item">
                                <div class="photo"><img alt="" src="<?=newthumbs($category['img'], 'category', 115, 115, '115x115x0', 0)?>"></div>
                                <p class="<?=(uri(3) == $category['uri']) ? 'active_category_thumb' : ''?>"><?=$category['title']?></p>
